...
def hist(
    data: Union[Sequence, Mapping[Any, int]],
    ...
    counts, bin_edges = np.histogram(
        data,
        weights=(np.ones(len(data)) / float(len(data)) if relative else None),
        range=(start - bin_size / 2, end + bin_size / 2),
        bins=bin_num,
    )
    if text is not None:
